/**
 * REST client for the hotel API implemented as Next.js Route Handlers (`src/app/api/[...path]`).
 * Browser calls use same-origin `/api/...`. Server-side helpers need an absolute API base URL.
 */

import { getStaffAuthHeaders } from "@/lib/auth/staff-session";

/**
 * Base URL for API routes (no trailing slash). Paths like `/bookings` are appended.
 * Set `BACKEND_URL` or `NEXT_PUBLIC_BACKEND_URL` to a full API root including `/api`, e.g. `http://127.0.0.1:3001/api`.
 * If unset, defaults to this app’s origin + `/api` (Vercel: `https://$VERCEL_URL/api` when `VERCEL_URL` is set).
 */
export function getBackendBaseUrl(): string {
  const explicit = (process.env.BACKEND_URL ?? process.env.NEXT_PUBLIC_BACKEND_URL ?? "").trim().replace(/\/$/, "");
...
/** Exact phrase required by POST `/settings/reset` (matches server Zod literal). */
export const SYSTEM_RESET_CONFIRM_PHRASE = "RESET SIU HOTEL" as const;

export type SystemResetDeletedCounts = {
  transactions: number;
  bookings: number;
  customers: number;
  rooms: number;
...
export async function resetSystemDataClient(): Promise<{
  deleted: SystemResetDeletedCounts;
  user: StaffUserDto;
}> {
  const res = await fetch(clientProxyPath("/settings/reset"), {
    method: "POST",
    headers: { "Content-Type": "application/json", ...getStaffAuthHeaders() },
    body: JSON.stringify({ confirmPhrase: SYSTEM_RESET_CONFIRM_PHRASE }),
  });
  return readSuccess(res, "System reset");
}

/**
 * Server-side fetch (absolute URL to this app’s `/api/...` handlers). Browser code uses {@link clientProxyPath}.
 */
