...
import type { BookingDto, HotelInfoDto, TransactionDto } from "@/lib/api/apiService";
import {
  ssrBookingById,
  ssrHotelInfoOrNull,
  ssrTransactionsList,
} from "@/server/ssr/dashboard-data";
...
export default async function BookingInvoicePage(props: PageProps) {
  ...
  const br = await ssrBookingById(id);
  booking = br.booking;
  loadError = br.loadError;

  try {
    transactions = await ssrTransactionsList();
  } catch {
    transactions = [];
  }
  hotelInfo = await ssrHotelInfoOrNull();
